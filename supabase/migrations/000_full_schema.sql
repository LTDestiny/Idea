-- ============================================
-- STEAM_TUI — Full Database Schema + Seed Data
-- Single-file migration: Schema + RLS + Functions + Triggers + Mock Data
-- ============================================
...
CREATE TYPE idea_category AS ENUM (
  'social', 'technology', 'education', 'health',
  'environment', 'business', 'art', 'other'
);
...
-- ideas (đã bao gồm looking_for thay cho required_skills)
CREATE TABLE ideas (
...
-- comments (đã bao gồm parent_id cho reply/thread)
...
CREATE INDEX idx_join_requests_idea      ON join_requests(idea_id);
CREATE INDEX idx_join_requests_requester ON join_requests(requester_id);

-- Unique constraint: one join request per user per idea
CREATE UNIQUE INDEX idx_join_requests_unique ON join_requests(idea_id, requester_id);
...
CREATE POLICY "ideas_insert" ON ideas FOR INSERT WITH CHECK (
  auth.role() = 'authenticated' AND creator_id = auth.uid()
);
CREATE POLICY "ideas_update" ON ideas FOR UPDATE USING (
  auth.role() = 'authenticated' AND creator_id = auth.uid()
);
CREATE POLICY "ideas_delete" ON ideas FOR DELETE USING (
  auth.role() = 'authenticated' AND creator_id = auth.uid()
);
...
-- Auto-update updated_at trên bảng ideas
...
-- Tự động tạo profile khi user đăng ký qua auth
...
-- ============================================
-- PHẦN 6: SEED DATA (Mock / Dữ liệu ảo)
-- ============================================
-- LƯU Ý: Chỉ dùng cho dev/test — KHÔNG dùng production

-- 6.1 Fake users trong auth.users
...
VALUES
  ('11111111-1111-1111-1111-111111111111', 'nguyenvanan@example.com',
   '{"full_name": "Nguyễn Văn An", "avatar_url": ""}',
   now(), now(), '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   '$2a$10$PznXqIEbCLR.XkP2gFJuPeVfBxZJHwM3pWy0O/4G6i1GfJJhRJZnG', now(), now()),

  ('22222222-2222-2222-2222-222222222222', 'tranthibinh@example.com',
   '{"full_name": "Trần Thị Bình", "avatar_url": ""}',
   now(), now(), '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   '$2a$10$PznXqIEbCLR.XkP2gFJuPeVfBxZJHwM3pWy0O/4G6i1GfJJhRJZnG', now(), now()),

  ('33333333-3333-3333-3333-333333333333', 'lehoangcuong@example.com',
   '{"full_name": "Lê Hoàng Cường", "avatar_url": ""}',
   now(), now(), '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   '$2a$10$PznXqIEbCLR.XkP2gFJuPeVfBxZJHwM3pWy0O/4G6i1GfJJhRJZnG', now(), now()),

  ('44444444-4444-4444-4444-444444444444', 'phamminhduong@example.com',
   '{"full_name": "Phạm Minh Dương", "avatar_url": ""}',
   now(), now(), '00000000-0000-0000-0000-000000000000', 'authenticated', 'authenticated',
   '$2a$10$PznXqIEbCLR.XkP2gFJuPeVfBxZJHwM3pWy0O/4G6i1GfJJhRJZnG', now(), now()),

  ('55555555-5555-5555-5555-555555555555', 'hoangthievy@example.com',
...
-- 6.3 Ideas (8 ý tưởng, đủ 8 category)
...
-- 6.4 Comments (23 bình luận: 19 top-level + 4 replies)
...
-- 6.5 Join Requests (12 yêu cầu tham gia)
...
-- ============================================
-- TỔNG KẾT DỮ LIỆU ẢO:
-- • 5 users (fake auth.users + profiles)
